...
real (kind=kdp) :: dimset                    ! dimension in which is embedded the dataset (real (kind=kdp))
...
integer (kind=idp) :: dimint                 ! integer (kind=idp) of dimset (avoid real (kind=kdp) calc)
...
if (id_err.ne.zero) call error_man (id_err)
call merging(id_err)                         ! Some preprocesing before merging
if (id_err.ne.zero) call error_man (id_err)
call graph2(id_err)                          ! Print graph after automatic merging
if (id_err.ne.zero) call error_man (id_err)
...
if (allocate_status /= 0) STOP "*** Not enough memory: line 199 of DPA.f90 ****"
    ...
    select case (dim_type)
      ...
      case (2)
        write (6,*) "give the dimension (integer)"
        read (5,*) dimint
      case default
        id_err=7
        return
...
write (51,*) "DIMENSION EMPLOYED:", dimint
return
111 id_err=2
return
112 id_err=3
...
write (6,*) "Provide the dimension (integer)"
read (5,*) dimint
deallocate (nu,i_nu)
return
...
if (allocate_status /= 0) STOP "*** Not enough memory: line 666 of DPA.f90 ****"
    do i=uno,Nele
      ...
      do j=uno,limit
        Nlist(i,j)=iVols(j)
        VV(i,j)=prefactor*Vols(iVols(j))**dimint
      enddo
    enddo
...
if (allocate_status /= 0) STOP "*** Not enough memory: line 853 of DPA.f90 ****"
    ...
    do i=1,Nele
      ...
      do j=1,limit
        Nlist(i,j)=iVols(j)
      enddo
      Rho(i)=prefactor-float(dimint)*log(Vols(iVols(kknn)))
...
write (6,*) "Provide the Z parameter"
read (5,*) Zmerge
...
! Change properties according with Pop_cut for visualization 
  
  do i=uno,Nclus_m
    if (Pop_m(i).lt.Pop_cut) then
      cent_m(i)=0.
      cent_err_m(i)=rzero
    endif
  enddo
  do i=uno,Nclus_m-uno
    do j=i+uno,Nclus_m
      if ((Pop_m(i).lt.Pop_cut).or.(Pop_m(j).lt.Pop_cut)) then
        Bord_m(i,j)=rzero
        Bord_err_m(i,j)=rzero
        Bord_m(j,i)=rzero
        Bord_err_m(j,i)=rzero
      endif
    enddo
  enddo
else
...
!
 ...
 real (kind=kdp) :: last_barrier       ! Last node in the dendrogramm
...
! dendro_active_barrier :: index of the active barrier
 ...
 open (21,file="Dendrogramm_graph_2.gpl")
 open (22,file="Dendrogramm_labels_2.dat")
...
! write (21,'(a,i6,a)') "pl [0:",Nele,"] [0:] 'Dendrogramm_labels_2.dat' u 1:2:3 w p pt 7 ps 2 lc palette z t '','' u 1:2:3 w labels t ''"
 write (21,'(a,i6,a,es17.10,a,es17.10,a)') "pl [0:",Nele,"] [",dendro_points(k,2)-real(0.05,kdp)*hdendro,":",maxdens+real(0.05,kdp)*hdendro,"] 'Dendrogramm_labels_2.dat' u 1:2:3 w p pt 7 ps 2 lc palette z t '','' u 1:2:3 w labels t ''"
!write (23,3333) Narrow,dendro_points(k,1),dendro_points(k,2),dendro_points(k,1),-1.,width_max+1.
 close (21)
 close (22)
! call execute_command_line("gnuplot -persists Dendrogramm_graph_2.gpl")
!
 return
