...
! Adding restart option
character (len=80):: arg1
character (len=80):: rstf
logical :: restart
integer (kind=idp):: i,j,ii,jj

restart=.false.
call getarg(1,arg1)
if (trim(arg1)=='rst') restart=.true.
...
if (restart) then
  write (6,*) "You are restarting a previous clustering"
  write (6,*) "Give me the name of the restart file (generated by the"
  write (6,*) "previous computing as Restart_summary.rst)"
  read (5,*) rstf
  open (555,file=rstf,status='old',err=5550)
  read (555,*) Nele
  read (555,*) Nclus
  allocate(Bord(Nclus,Nclus),stat=allocate_status)
  if (allocate_status /= 0) then
    id_err=18
    call error_man (id_err)
  endif
  allocate(Bord_err(Nclus,Nclus),stat=allocate_status)
  if (allocate_status /= 0) then
    id_err=18
    call error_man (id_err)
  endif
  do i=1,Nclus
    do j=1,Nclus
       read(555,*) ii,jj,Bord(ii,jj),Bord_err(ii,jj)
    enddo
  enddo
  ...
  do i=1,Nele
    read (555,'(i6,1x,i6,1x,3(es18.10,1x),i6)') ii,Nstar(i),dc(i),Rho(i),Rho_err(i),Cluster(i)
  enddo
  ...
  do i=1,Nclus
    read (555,*) cent(i),cent_err(i),Centers(i)
  enddo
  close (555)
else
...
select case (id_err)
  ...
  case (18) 
    ...
    call exit (18)
  case (19) 
    write (6,*) "Error opening restart file"
    call exit (19)
  case default
...
enddo
open (555,file='Restart_summary.rst')
write (555,*) Nele
write (555,*) Nclus
do i=1,Nclus
  do j=1,Nclus
    write (555,*) i,j,Bord(i,j),Bord_err(i,j)
  enddo
enddo
do i=1,Nele
  write (555,'(i6,1x,i6,1x,3(es18.10,1x),i6)') i,Nstar(i),dc(i),Rho(i),Rho_err(i),Cluster(i)
enddo
do i=1,Nclus
  write (555,*) cent(i),cent_err(i),Centers(i)
enddo
close (555)
else
  Cluster(:)=uno
